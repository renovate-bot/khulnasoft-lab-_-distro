...
```
mkdir -p $GOPATH/src/github.com/ansible-distro && cd $GOPATH/src/github.com/ansible-distro
```

2) Clone distro (with submodules)

```
git clone --recursive git@github.com:khulnasoft-lab/distro.git && cd distro
```
...
4) Create database if you want to use MySQL (Distro also supports [bbolt](https://github.com/etcd-io/bbolt), it doesn't require additional action) 

```
echo "create database distro;" | mysql -uroot -p
```
...
2) Install Dredd globally
    ...
    npm install -g dredd
    ```
3) Create `./dredd/config.json` for Dredd. It must contain database connection same as used in Distro server.
    ...
    ```json
   {
    ...
    ```
4) Start Distro server (add `--config` option if required):
    ````bash
    ./bin/distro server
    ```
5) Start Dredd tests
