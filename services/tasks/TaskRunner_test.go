...
func CreateBoltDB() db.Store {
	...
	fn := "/tmp/test_distro_db_" + strconv.Itoa(r.Int())
	store := bolt.BoltDb{
		Filename: fn,
	}
	return &store
...
func TestTaskGetPlaybookArgs(t *testing.T) {
	...
	if res != "-i /tmp/inventory_0 --extra-vars {\"distro_vars\":{\"task_details\":{\"id\":0,\"username\":\"\"}}} test.yml" {
...
func TestTaskGetPlaybookArgs2(t *testing.T) {
	...
	res := strings.Join(args, " ")
	if res != "-i /tmp/inventory_0 --extra-vars=@/tmp/access_key_0 --extra-vars {\"distro_vars\":{\"task_details\":{\"id\":0,\"username\":\"\"}}} test.yml" {
		t.Fatal("incorrect result")
	}
}
