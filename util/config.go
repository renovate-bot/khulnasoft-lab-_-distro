...
func loadConfigFile(configPath string) {
	if configPath == "" {
		configPath = os.Getenv("DISTRO_CONFIG_PATH")
	}
	...
	if configPath == "" {
		...
		paths := []string{
			...
			"/usr/local/etc/distro/config.json",
		}
		for _, p := range paths {
			_, err = os.Stat(p)
			if err != nil {
...
func exitOnConfigFileError(err error) {
	if err != nil {
		exitOnConfigError("Cannot Find configuration! Use --config parameter to point to a JSON file generated by `distro setup`.")
	}
}
...
func CheckUpdate() (updateAvailable *github.RepositoryRelease, err error) {
	...
	releases, _, err := gh.Repositories.ListReleases(context.TODO(), "ansible-distro", "distro", nil)
	if err != nil {
		return
	}
...
func (d *DbConfig) GetDbName() string {
	dbName := os.Getenv("DISTRO_DB_NAME")
	if dbName != "" {
		return dbName
	}
	return d.DbName
...
func (d *DbConfig) GetUsername() string {
	username := os.Getenv("DISTRO_DB_USER")
	if username != "" {
		return username
	}
	return d.Username
...
func (d *DbConfig) GetPassword() string {
	password := os.Getenv("DISTRO_DB_PASS")
	if password != "" {
		return password
	}
	return d.Password
...
func (d *DbConfig) GetHostname() string {
	hostname := os.Getenv("DISTRO_DB_HOST")
	if hostname != "" {
		return hostname
	}
	return d.Hostname
