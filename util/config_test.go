...
func TestLoadConfigEnvironmet(t *testing.T) {
	...
	os.Setenv("DISTRO_PORT", envPort)
	os.Setenv("DISTRO_COOKIE_HASH", envCookieHash)
	os.Setenv("DISTRO_ACCESS_KEY_ENCRYPTION", envAccessKeyEncryption)
	os.Setenv("DISTRO_MAX_PARALLEL_TASKS", envMaxParallelTasks)
	os.Setenv("DISTRO_LDAP_NEEDTLS", envLdapNeedTls)
	os.Setenv("DISTRO_DB_HOST", envDbHost)
