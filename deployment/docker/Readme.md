...
Production images for each tag, latest and the development branch will be pushed to [docker hub](https://hub.docker.com/r/khulnasoft/distro).
...
this will create an image called `khulnasoft/distro:latest` which will be compiled from the currently checked out code
...
To run Distro in a simple production-like docker configuration run the following command:

    task dc:prod

You can then access Distro directly from the url http://localhost:8081/
...
Run `task dc:prod` and your Distro instance will then be at the url
...
`dc:dev` rebuilds the development images and runs a development stack, with the distro root as a volume link
This allows you to work inside the container with live code. The container has all the tools you need to build and test distro
