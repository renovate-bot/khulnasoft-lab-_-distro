...
func doUpgrade() error {
	...
	// create tmp file
	tmpPath := filepath.Join(os.TempDir(), "distro-upgrade")
	...
	// replace it
	cmdPath := util.FindDistro()
	if len(cmdPath) == 0 {
		return errors.New("cannot find distro binary")
	}
	...
	fmt.Println("visit https://github.com/khulnasoft-lab/distro/releases for the changelog")
	go func() {
...
func findAsset(release *github.RepositoryRelease) *github.ReleaseAsset {
	for _, asset := range release.Assets {
		...
		if strings.HasPrefix(*asset.Name, "distro_") &&
