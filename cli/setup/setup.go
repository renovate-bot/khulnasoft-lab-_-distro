...
4. Set up initial distro user & password
...
`
	...
	defaultPlaybookPath := filepath.Join(os.TempDir(), "distro")
	askValue("Playbook path", defaultPlaybookPath, &conf.TmpPath)
	conf.TmpPath = filepath.Clean(conf.TmpPath)

	askValue("Public URL (optional, example: https://example.com/distro)", "", &conf.WebHost)
	...
	if conf.EmailAlert {
		...
		askValue("Mail sender address", "distro@localhost", &conf.EmailSender)
	}
...
func scanMySQL(conf *util.ConfigType) {
	...
	askValue("db Name", "distro", &conf.MySQL.DbName)
}
...
func scanPostgres(conf *util.ConfigType) {
	...
	askValue("db Name", "distro", &conf.Postgres.DbName)
...
func SaveConfig(config *util.ConfigType) (configPath string) {
	...
	if err != nil {
		...
		if err != nil {
			// Final fallback
			configDirectory = "/etc/distro"
		}
		configDirectory = filepath.Join(configDirectory, "distro")
	}
